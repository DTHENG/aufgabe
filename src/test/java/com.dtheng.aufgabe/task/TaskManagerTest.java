...
public class TaskManagerTest {
    ...
    public void testCreate() throws Exception {
        ...
        Task task = taskManager.create(createRequest).map(AggregateTask::getTask).toBlocking().single();
        log.debug("task: {}", task);
    ...
    public void testGet() throws Exception {
        ...
        createRequest.setDescription("test task");
        Task task = taskManager.create(createRequest).map(AggregateTask::getTask).toBlocking().single();
        Task gotTask = taskManager.get(task.getId()).map(AggregateTask::getTask).toBlocking().single();
    ...
    public void testLookup() throws Exception {
        ...
        taskManager.create(createRequest1).map(AggregateTask::getTask).toBlocking().single();
        TaskCreateRequest createRequest2 = new TaskCreateRequest();
        createRequest2.setDescription("test task #2");
        taskManager.create(createRequest2).map(AggregateTask::getTask).toBlocking().single();
        TaskCreateRequest createRequest3 = new TaskCreateRequest();
        createRequest3.setDescription("test task #3");
        taskManager.create(createRequest3).map(AggregateTask::getTask).toBlocking().single();
        AggregateTasksResponse response = taskManager.get(new TasksRequest()).toBlocking().single();
    ...
    public void testUpdate() throws Exception {
        ...
        Task updatedTask = taskManager.update(task.getId(), updateRequest).map(AggregateTask::getTask).toBlocking().single();
